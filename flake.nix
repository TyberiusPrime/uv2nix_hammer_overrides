  ...
  outputs =
    ...
    let
      ...
      version_match_no_fallback =
        ...
        in
        if builtins.length older_versions == 0 then null else lib.lists.last older_versions;

      matched_overrides =
        debug: version_match: nixpkgs_pkgs: final: prev:
        ...
        // {
          cython_0 = prev.cython.overrideAttrs (old: {
            src =
              if
                ((builtins.trace (old.passthru.format or "sdist") (old.passthru.format or "sdist")) == "wheel")
              then
    ...
    in
    {
      ...
      overrides = (matched_overrides false) version_match;
      overrides_debug = (matched_overrides true) version_match;
      overrides_strict = (matched_overrides false) version_match_no_fallback;
      overrides_strict_debug = (matched_overrides true) version_match_no_fallback;
